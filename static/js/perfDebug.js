/**
 * Devante CLI IDE / chat performance debugger.
 *
 ...
 *
 * Commands (global: window.__devanteCliPerfDebug):
...
(function () {
  ...
  if (window.__devanteCliPerfDebug && window.__devanteCliPerfDebug._running) {
    console.warn('[perfDebug] already running — call __devanteCliPerfDebug.report()');
    return;
  }

  const START = performance.now();
  const LOAD_AGE_MS = Date.now() - (window._devanteCliLoadTime || Date.now());
  ...
  function report() {
    ...
    console.group('%c Devante CLI perf debug report ', 'background:#e06c75;color:#fff;font-weight:bold;padding:2px 6px');
    ...
    console.group('Tips');
    console.log('1. Reproduce slowness, then run __devanteCliPerfDebug.report() again');
    console.log('2. __devanteCliPerfDebug.traceNext(5) then switch tabs to capture call stacks');
    console.log('3. Chrome Performance tab → record while switching session / streaming');
    console.log('4. __devanteCliPerfDebug.reportHtml() for live overlay');
    console.groupEnd();
  ...
  function reportHtml() {
    ...
    function render() {
      ...
      panelEl.innerHTML = [
        ...
        '<strong style="color:#e06c75">Devante CLI perf</strong>',
        '<span>',
  ...
  function start() {
    ...
    domSnapshot('start');
    console.info(
      '%c[perfDebug] active%c — use __devanteCliPerfDebug.report() or .reportHtml()',
  ...
  window.__devanteCliPerfDebug = api;
  start();
})();
