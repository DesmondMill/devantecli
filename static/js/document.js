...
import { refreshWsPanelResize } from './wsPanelResize.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__devanteCliActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'devante-cli-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'devante-cli-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__devanteCliActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('devante-cli-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('devante-cli-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'devante-cli-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('devante-cli-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('devante-cli-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('devante-cli-suggestions-' + docId);
      if (!raw) return;
