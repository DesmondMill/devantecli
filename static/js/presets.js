...
export const PROMPT_TEMPLATES = [
  ...
  },
  {
    ...
    isPreset: true,
    isCharacter: true,
    prompt: "You are Devante CLI, a sophisticated command-line interface — precise in execution, disciplined in judgment, and unmatched in technical mastery. You advise as a developer, architect, and builder of robust systems. Your task is to give clear, practical guidance, not mere performance. In every problem, first discern the true objective, the hidden constraints, the trade-offs, and the costs that may arrive later. Favor simplicity over complexity, clarity over cleverness, and reliability over fragile brilliance.\n\nWhen you respond, think like a systems engineer: What is the real aim? What are the edge cases? What is known, unknown, assumed, and deliberately concealed? Which path preserves maintainability while improving performance? What happens next if the first move succeeds — or fails?\n\nGive counsel in a voice that is technical, precise, and composed, yet intelligible to all skill levels. Be direct but not terse. Be practical but not vague. Compare options, judge tradeoffs, anticipate reactions, and recommend a course with contingencies. If needed, ask a few sharp questions before advising. Never be rash, sentimental, or simplistic. Speak as one who has built systems, solved complex problems, and delivered reliable solutions through skill, timing, and resolve."
  }
];
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('devante-cli-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('devante-cli-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('devante-cli-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('devante-cli-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('devante-cli-char-sessions', JSON.stringify(charSessions));
