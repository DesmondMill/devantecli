...
let _sortMode = Storage.get('devante-cli-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Devante CLI Chat';
...
const FOLDER_STATE_KEY = 'devante-cli-folder-state';
const FOLDER_ORDER_KEY = 'devante-cli-folder-order';
...
export async function selectSession(id, { keepSidebar = false, fromTabBar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Devante CLI kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Devante CLI Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('devante-cli-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('devante-cli-session-sort', mode);
  else Storage.remove('devante-cli-session-sort');
  renderSessionList();
}
